...
loco()
//////////////////////////////////////
//새로고침 시 화면 깨지는 현상을 막기 위해
function scrollToTop(){
 window.scrollTo(0,0)
}
window.onload=function(){
  //setTimeout(할일,시간) //시간이 지난 후에 할일
...
let clutter = "";
//textContent --> 텍스트만 추출
let pag2_h2 = document.querySelector("#page2>h2").textContent.split("");
console.log(pag2_h2)
pag2_h2.forEach(function (dets) {
  clutter += `<span>${dets}</span>`; //clutter = clutter + `<span>a</span>`
...
gsap.to("#page2>h2>span", {
  scrollTrigger: {
    ...
    scroller: "#main", //locomotive 사용 시 필수
    scrub: 0.5
  },
  color: "#fff",
  stagger: 0.2
...
function canvas() {
  ...
  gsap.to(imgeSeq,{
    ...
    scrollTrigger:{
      ...
      scroller:"#main" //locomotive에서 scroll을 감지하는 역할
    },
    onUpdate:render
  })
  images[0].onload=render;
  function render(){
    scaleImage(images[imgeSeq.frame],context)
  }
...
let clutter2 = "";
//textContent --> 텍스트만 추출
let pag4_h2 = document.querySelector("#page4>h2").textContent.split("");
console.log(pag2_h2)
pag4_h2.forEach(function (dets) {
...
let clutter3 = "";
//textContent --> 텍스트만 추출
let pag6_h2 = document.querySelector("#page6>h2").textContent.split("");
pag6_h2.forEach(function (dets) {
